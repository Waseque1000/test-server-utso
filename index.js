...
async function run() {
  try {
    ...
    app.get("/recent-artworks", async (_, res) => {
    ...
    app.get("/artwork/:id", async (req, res) => {
      const result = await artworks.findOne({ _id: new ObjectId(req.params.id) });
      res.send(result);
    });
    ...
    app.put("/update-artwork/:id", async (req, res) => {
      ...
      const result = await artworks.updateOne({ _id: new ObjectId(id) }, updateDoc, { upsert: true });
      res.send(result);
    });

    app.delete("/artwork/:id", async (req, res) => {
      const result = await artworks.deleteOne({ _id: new ObjectId(req.params.id) });
      res.send(result);
    });

    // --- USER GALLERY & STATS ---
    app.get("/my-gallery/:email", async (req, res) => {
      const result = await artworks.find({ userEmail: req.params.email }).toArray();
      res.send(result);
    });

    app.get("/user-total-art/:email", async (req, res) => {
      const totalPosts = await artworks.countDocuments({ userEmail: req.params.email });
      res.send({ totalPosts });
    });
    ...
    app.post("/favorites", async (req, res) => {
      ...
      if (exists) return res.status(400).send({ success: false, message: "Already in favorites" });
    ...
    app.get("/favorites/:email", async (req, res) => {
      const result = await favorites.find({ userEmail: req.params.email }).toArray();
      res.send(result);
    });
    ...
    // --- ARTIST DETAILS ---
    app.get("/artist-details/:email", async (req, res) => {
      const artworksList = await artworks.find({ userEmail: req.params.email }).toArray();
      const artist = {
        name: artworksList[0]?.userName || "Unknown Artist",
        image: artworksList[0]?.userImage || "https://i.ibb.co/Vj1qCmh/image-2.png",
        bio: "Passionate about guitars and fine arts. Creating music through visuals.",
        totalArtworks: artworksList.length,
        followers: 120,
      };
      res.send({ artist, artworks: artworksList });
    });
